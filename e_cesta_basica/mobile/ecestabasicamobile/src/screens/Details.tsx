...
export function Details() {
  ...
  const [isPlaceholderVisible1, setPlaceholderVisible1] = useState(false);
  const [isPlaceholderVisible2, setPlaceholderVisible2] = useState(false);
  const [isPlaceholderVisible3, setPlaceholderVisible3] = useState(false);
  const [isPlaceholderVisible4, setPlaceholderVisible4] = useState(false);
  const [isPlaceholderVisible5, setPlaceholderVisible5] = useState(false);
  const [isPlaceholderVisible6, setPlaceholderVisible6] = useState(false);
  const [isPlaceholderVisible7, setPlaceholderVisible7] = useState(false);
  const [isPlaceholderVisible8, setPlaceholderVisible8] = useState(false);
  const [isPlaceholderVisible9, setPlaceholderVisible9] = useState(false);
  const [isPlaceholderVisible10, setPlaceholderVisible10] = useState(false);
  const [isPlaceholderVisible11, setPlaceholderVisible11] = useState(false);
  const [isPlaceholderVisible12, setPlaceholderVisible12] = useState(false);
  const [isPlaceholderVisible13, setPlaceholderVisible13] = useState(false);
  const [isPlaceholderVisible14, setPlaceholderVisible14] = useState(false);
  const [isPlaceholderVisible15, setPlaceholderVisible15] = useState(false);
  const [isPlaceholderVisible16, setPlaceholderVisible16] = useState(false);
  const [isPlaceholderVisible17, setPlaceholderVisible17] = useState(false);
  const [isPlaceholderVisible18, setPlaceholderVisible18] = useState(false);
  const [isPlaceholderVisible19, setPlaceholderVisible19] = useState(false);
  const [isPlaceholderVisible20, setPlaceholderVisible20] = useState(false);
  const [isPlaceholderVisible21, setPlaceholderVisible21] = useState(false);
  const [isPlaceholderVisible22, setPlaceholderVisible22] = useState(false);
  const [isPlaceholderVisible23, setPlaceholderVisible23] = useState(false);
  const [isPlaceholderVisible24, setPlaceholderVisible24] = useState(false);
  const [isPlaceholderVisible25, setPlaceholderVisible25] = useState(false);
  const [isPlaceholderVisible26, setPlaceholderVisible26] = useState(false);
  const [isPlaceholderVisible27, setPlaceholderVisible27] = useState(false);
  const [isPlaceholderVisible28, setPlaceholderVisible28] = useState(false);
  ...
  async function handleUpdatePesquisa() {

    try {
      setIsLoading(true);
      const nSupermercado = nomeSupermercado
      const mAno = mesAno
      ...
      await api.put(`/editarPesquisaCompleta/${id}`, {
        mes_ano: mAno,
        nome_supermercado: nSupermercado,
      ...
      toast.show({
        title: "Supermercado cadastrado com sucesso",
        placement: "top",
        bgColor: "green.500",
      })
    } catch (error) {
      console.log(error);
      toast.show({
        title: "Não foi possível editar a Pesquisa",
        placement: "top",
        bgColor: "red.500",
      })
    } finally {
  ...
  async function fetchPesquisaDetails() {
    try {
      ...
      const pesquisaDetails = response.data.pesquisas[0]; // Obtém o objeto com os detalhes da pesquisa
      const mAno = pesquisaDetails.mes_ano;
      const nSupermercado = pesquisaDetails.nome_supermercado;
      const carneBovinaChaDentro = pesquisaDetails.carneBovinaChaDentro;
      const carneBovinaChaFora = pesquisaDetails.carneBovinaChaFora
      const carneBovinaPatinhoCoxaoMole = pesquisaDetails.carneBovinaPatinhoCoxaoMole
      const leiteIntegralValeDourado = pesquisaDetails.leiteIntegralValeDourado
      const leiteIntegralPiracanjuba = pesquisaDetails.leiteIntegralPiracanjuba
      const leiteIntegralSabugi = pesquisaDetails.leiteIntegralSabugi
      const feijaoCariocaUrbano = pesquisaDetails.feijaoCariocaUrbano
      const feijaoCariocaDuPrato = pesquisaDetails.feijaoCariocaDuPrato
      const feijaoCariocaCunhau = pesquisaDetails.feijaoCariocaCunhau
      const arrozParboilizadoChines = pesquisaDetails.arrozParboilizadoChines
      const arrozParboilizadoFortelli = pesquisaDetails.arrozParboilizadoFortelli
      const arrozParboilizadoUrbano = pesquisaDetails.arrozParboilizadoUrbano
      const farinhaMandiocaQuentinha = pesquisaDetails.farinhaMandiocaQuentinha
      const farinhaMandiocaCurimatau = pesquisaDetails.farinhaMandiocaCurimatau
      const farinhaMandiocaDuPrato = pesquisaDetails.farinhaMandiocaDuPrato
      const pao = pesquisaDetails.pao
      const tomate = pesquisaDetails.tomate
      const cafePoSaoBraz = pesquisaDetails.cafePoSaoBraz
      const cafePoSantaClara = pesquisaDetails.cafePoSantaClara
      const cafePoNordestino = pesquisaDetails.cafePoNordestino
      const acucarNectar = pesquisaDetails.acucarNectar
      const acucarPuroMel = pesquisaDetails.acucarPuroMel
      const bananaPrata = pesquisaDetails.bananaPrata
      const bananaPacovan = pesquisaDetails.bananaPacovan
      const oleoSojaSoya = pesquisaDetails.oleoSojaSoya
      const oleoSojaPrimor = pesquisaDetails.oleoSojaPrimor
      const manteigaSaborosa = pesquisaDetails.manteigaSaborosa
      const manteigaJucurutu = pesquisaDetails.manteigaJucurutu
      const manteigaTerra = pesquisaDetails.manteigaTerra

      setMesAno(mAno);
      setNomeSupermercado(nSupermercado);
      setCarneBovinaChaFora(((carneBovinaChaFora / 100).toFixed(2).padStart(4, '0')).toString())
      setCarneBovinaPatinhoCoxaoMole(((carneBovinaPatinhoCoxaoMole / 100).toFixed(2).padStart(4, '0')).toString())
      setLeiteIntegralValeDourado(((leiteIntegralValeDourado / 100).toFixed(2).padStart(4, '0')).toString())
      setLeiteIntegralPiracanjuba(((leiteIntegralPiracanjuba / 100).toFixed(2).padStart(4, '0')).toString())
      setLeiteIntegralSabugi(((leiteIntegralSabugi / 100).toFixed(2).padStart(4, '0')).toString())
      setFeijaoCariocaUrbano(((feijaoCariocaUrbano / 100).toFixed(2).padStart(4, '0')).toString())
      setFeijaoCariocaDuPrato(((feijaoCariocaDuPrato / 100).toFixed(2).padStart(4, '0')).toString())
      setFeijaoCariocaCunhau(((feijaoCariocaCunhau / 100).toFixed(2).padStart(4, '0')).toString())
      setArrozParboilizadoChines(((arrozParboilizadoChines / 100).toFixed(2).padStart(4, '0')).toString())
      setArrozParboilizadoFortelli(((arrozParboilizadoFortelli / 100).toFixed(2).padStart(4, '0')).toString())
      setArrozParboilizadoUrbano(((arrozParboilizadoUrbano / 100).toFixed(2).padStart(4, '0')).toString())
      setFarinhaMandiocaQuentinha(((farinhaMandiocaQuentinha / 100).toFixed(2).padStart(4, '0')).toString())
      setFarinhaMandiocaCurimatau(((farinhaMandiocaCurimatau / 100).toFixed(2).padStart(4, '0')).toString())
      setFarinhaMandiocaDuPrato(((farinhaMandiocaDuPrato / 100).toFixed(2).padStart(4, '0')).toString())
      setPao(((pao / 100).toFixed(2).padStart(4, '0')).toString())
      setTomate(((tomate / 100).toFixed(2).padStart(4, '0')).toString())
      setCafePoSaoBraz(((cafePoSaoBraz / 100).toFixed(2).padStart(4, '0')).toString())
      setCafePoSantaClara(((cafePoSantaClara / 100).toFixed(2).padStart(4, '0')).toString())
      setCafePoNordestino(((cafePoNordestino / 100).toFixed(2).padStart(4, '0')).toString())
      setAcucarNectar(((acucarNectar / 100).toFixed(2).padStart(4, '0')).toString())
      setAcucarPuroMel(((acucarPuroMel / 100).toFixed(2).padStart(4, '0')).toString())
      setBananaPrata(((bananaPrata / 100).toFixed(2).padStart(4, '0')).toString())
      setBananaPacovan(((bananaPacovan / 100).toFixed(2).padStart(4, '0')).toString())
      setOleoSojaSoya(((oleoSojaSoya / 100).toFixed(2).padStart(4, '0')).toString())
      setOleoSojaPrimor(((oleoSojaPrimor / 100).toFixed(2).padStart(4, '0')).toString())
      setManteigaSaborosa(((manteigaSaborosa / 100).toFixed(2).padStart(4, '0')).toString())
      setManteigaJucurutu(((manteigaJucurutu / 100).toFixed(2).padStart(4, '0')).toString())
      setManteigaTerra(((manteigaTerra / 100).toFixed(2).padStart(4, '0')).toString())
      setCarneBovinaChaDentro(((carneBovinaChaDentro / 100).toFixed(2).padStart(4, '0')).toString());
